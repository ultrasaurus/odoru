...
def adjust_voice_speed(audio_np: np.ndarray, speed_factor: float) -> np.ndarray:
    """Time-stretch a reference voice clip to alter cloned speaking rate.

    Linear-interpolation resampling: speed_factor < 1 lengthens the clip
    (slower, lower-pitched reference -> slower clone), > 1 shortens it.
    Pitch is not preserved (like changing tape speed). Mirrors the approach
    used by VibeVoice-ComfyUI's voice_speed_factor; applied to the reference
    audio before the processor, since the model has no native speed hook.
    """
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Sampling temperature. When set, enables do_sample; when omitted, "
             "generation is greedy/deterministic (default).",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=1.0,
        help="Voice speed factor applied to the reference audio (not the model). "
             "<1 slows the cloned voice, >1 speeds it up; 1.0 = unchanged (default).",
    )
...
def main():
    ...
    # Apply voice speed factor to the reference audio (preprocessing). The
    # model has no native speed control, so we time-stretch the cloning
    # reference instead. Only done when speed != 1.0 so the default path
    # keeps passing plain file paths (unchanged behaviour). Load via the
    # processor's own loader so sample rate matches what it expects.
    if args.speed != 1.0:
        print(f"Applying voice speed factor: {args.speed}")
        voice_samples = [
            adjust_voice_speed(
                processor.audio_processor._load_audio_from_path(path),
                args.speed,
            )
    ...
    # Temperature only takes effect when sampling; otherwise stay greedy
    # (deterministic), preserving the original default behaviour.
    if args.temp is not None:
        generation_config = {'do_sample': True, 'temperature': args.temp}
        print(f"Starting generation with cfg_scale: {args.cfg_scale}, temperature: {args.temp}")
    else:
