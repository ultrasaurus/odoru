...
export class Player {
     ...
     * then regardless.
     */
    listenTo(segIndex, endOffsetSecs, startOffsetSecs = 0, endSegIndex = segIndex) {
        if (!this.hasAudio || segIndex >= this.segments.length || endSegIndex >= this.segments.length)
            return;
