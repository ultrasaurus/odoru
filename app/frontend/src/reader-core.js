...
// Byline formatting
// ---------------------------------------------------------------------------
export function formatByline(authors, date) {
...
export class ReaderCore {
    ...
    outlineEls = [];
    activeOutlineIdx = -1;
    constructor(_transcriptEl, outlineEl) {
     ...
     */
    loadSpans(spans, interactive, onSeek) {
        this.spans = spans;
     ...
     */
    updateOutlineActive(position, getTime) {
        let found = -1;
        ...
        if (found === this.activeOutlineIdx)
            return;
        if (this.activeOutlineIdx >= 0)
            this.outlineEls[this.activeOutlineIdx]?.classList.remove('active');
        this.activeOutlineIdx = found;
        if (found >= 0)
            this.outlineEls[found]?.classList.add('active');
    }
}
