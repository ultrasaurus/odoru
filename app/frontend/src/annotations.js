...
const COLOR_ORDER = ['yellow', 'coral', 'mint', 'blue', 'lavender'];
function generateId() {
...
// ── DOM helpers ──────────────────────────────────────────────────────────────
function makeContext(segText, matchStart, matchLen) {
...
export async function applyAnnotations(container, docId) {
...
function showContextMenu(x, y, annId, container, getDocId) {
    hideContextMenu();
    ...
    del.addEventListener('mousedown', e => {
        ...
        if (docId)
            deleteAnnotation(container, docId, annId);
        hideContextMenu();
    });
...
export function initAnnotationPicker(articleArea, getDocId, isReadMode, getVoice) {
    ...
    picker.innerHTML = COLOR_ORDER.map(c => `<button class="annotation-swatch annotation-swatch-${c}" data-color="${c}" title="${c}"></button>`).join('');
    ...
    articleArea.addEventListener('contextmenu', e => {
        const mark = e.target.closest('.annotation');
    ...
    document.addEventListener('mousedown', e => {
        if (pickerEl && pickerEl.style.display !== 'none' && !pickerEl.contains(e.target)) {
            hidePicker();
        }
...
function showPicker(rect) {
    ...
    pickerEl.querySelectorAll('[data-color]').forEach(btn => {
