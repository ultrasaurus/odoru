...
function populateSidebar() {
    ...
    const sorted = [...data.manifest].sort((a, b) => a.title.localeCompare(b.title, undefined, { sensitivity: 'base' }));
...
function initPlayer(slug, transcript) {
    player = {
        slug,
        transcript,
...
function audioPath(slug, index) {
    return `documents/${slug}/audio/${String(index).padStart(4, '0')}.mp3`;
}
function startPrefetch(fromIndex) {
    if (!player)
        return;
    player.abortController?.abort();
    ...
    const prefetchNext = () => {
        if (!player || ac.signal.aborted)
            return;
        const i = player.prefetchOffset;
        if (i >= player.transcript.length)
            return;
        if (i >= player.currentIndex + PREFETCH_WINDOW)
            return;
