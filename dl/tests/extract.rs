use dl::{extract, OutputFormat};

#[test]
fn test_extract_davidtemkin() {
    let html = include_str!("fixtures/davidtemkin.html");
    let article = extract(html, "https://davidtemkin.com/about-mesa/", OutputFormat::Markdown)
...
fn test_extract_ultrasaurus() {
    ...
    let article = extract(html, "https://ultrasaurus.com/2019/04/when-reality-is-broken-change-the-rules/", OutputFormat::Markdown)
        .expect("extraction should succeed");
...
fn test_extract_text_format() {
    let html = include_str!("fixtures/davidtemkin.html");
    let article = extract(html, "https://davidtemkin.com/about-mesa/", OutputFormat::Text)
        .expect("extraction should succeed");
